...
def linear_term(q_hat,DPSI_bottom_hat,DPSI_top_hat):
    ...
    nlt_bottom = inverse_transform(v_hat[:,:,0]) * (-Umax/Lz)
        
    nlt_top = U0[-1] * inverse_transform((1j*kx*DPSI_top_hat.T).T)
    nlt_top += inverse_transform(v_hat[:,:,-1]) * (-Umax/Lz)
...
def nonlinear_term(q_hat,DPSI_bottom_hat,DPSI_top_hat):
    ...
    nlt_bottom += inverse_transform(v_hat[:,:,0]) * (inverse_transform(1j*ky*DPSI_bottom_hat)-Umax/Lz)
        
    nlt_top = inverse_transform(u_hat[:,:,-1]) * inverse_transform((1j*kx*DPSI_top_hat.T).T)
    nlt_top += inverse_transform(v_hat[:,:,-1]) * (inverse_transform(1j*ky*DPSI_top_hat)-Umax/Lz)
